		...
		case TIMER_CHECK:
			if (!m_busy) {
				...
				if (bytes > 0) {
					...
					timer_set(attotime::from_usec(5), TIMER_STROBE, 1);
				} else {
...
static MACHINE_CONFIG_START( dmcntr )
...
COMP( 2014, dmcntr, 0, 0, dmcntr, 0, dmcntr_state, 0, "Dummy Centronics", "Dummy Centronics", MACHINE_NOT_WORKING | MACHINE_NO_SOUND )
