...
void e05a30_device::update_printhead(int pos, UINT8 data)
{
	...
	if (pos == 0) {
		...
		m_printhead &= 0x00ff;
		m_printhead |= (UINT16) data << 1; /* MSB is pin 9 */
	}
}
 ...
 * For the CR motor, it is necessary to translate the input data from the
 * SLA7020M format to a format describing the 4 phases of a stepper motor.
 */
